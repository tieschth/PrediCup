...
Групповой этап: исход **П1 / Ничья / П2** (1 балл). Плей-офф: 7 вариантов —
победа/ничья в основное время (1), победа в доп. время или по пенальти (2).

## Возможности

- 🗳 Голосование на inline-кнопках с флагами — выбор виден только боту (анонимно
  для остальных), запись приватная через `callback_query`. На плей-офф — 7 кнопок
  (основное время / доп. время / пенальти).
- 🔁 Переголосование до начала матча, автозакрытие приёма ровно на старте.
- 🏁 Авто-резолв результата из внешнего источника и начисление очков (с учётом
  доп. времени и серии пенальти).
- 🔎 Авто-перепроверка завершённых матчей: если API досылает корректировку счёта
  (напр., отменённый на 90+ гол), бот пересчитывает очки и публикует поправку.
- 🏆 Таблица лидеров (`/leaderboard`) — только для админов; показывает всех, кто
  голосовал. Имена берутся из заданных меток (label), затем имени ТГ / @username.
- ✏️ Ручная корректировка очков (`scripts/adjust_points.py`) и отображаемые имена
  (`scripts/export_users.py`, `scripts/set_labels.py`).
...
- 🧪 Дев-режим: `/devmatch`, `/devopen`, `/devresult` (с `ET`/`PEN`), `/devreset`.
